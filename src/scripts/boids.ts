...
const setMagnitude = (vector: Vector, value: number): Vector => multiply(normalize(vector), value)
...
const cssColorToRgb = (value: string, fallback: string): [number, number, number] => {
    ...
    return [Number.parseInt(matches[0]!, 10), Number.parseInt(matches[1]!, 10), Number.parseInt(matches[2]!, 10)]
}
...
export const mountBoids = (root: HTMLElement) => {
    ...
    const createBoid = (): Boid => ({
        ...
        velocity: setMagnitude(createVector(Math.random() * 2 - 1, Math.random() * 2 - 1), 0.85 + Math.random() * 0.65),
    ...
    const syncPointer = (event: PointerEvent) => {
        ...
        pointer = x < 0 || y < 0 || x > rect.width || y > rect.height ? null : createVector(x, y)
    }
    ...
    const draw = () => {
        ...
        animatedMouseRadius = lerp(animatedMouseRadius, mouseRadiusTarget, isPointerPressed ? 0.18 : 0.14)
                ...
                if (d < boidsConfig.connectionRadius) {
                    const pointerBoost = pointer
                        ? Math.max(
                              0,
                              1 -
                                  Math.min(distance(boid.position, pointer), distance(other.position, pointer)) /
                                      animatedMouseRadius
                          )
                        : 0
        ...
        for (const boid of boids) {
            ...
            const pointerBoost = pointer ? Math.max(0, 1 - distance(boid.position, pointer) / animatedMouseRadius) : 0
